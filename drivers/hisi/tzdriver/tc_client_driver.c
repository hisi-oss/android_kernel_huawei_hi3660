...
#if 0
void dump_hash(char *pkname, unsigned char *hash_buf)
{
...
void spoof_hash(char *pkname, unsigned char *hash_buf)
{
	unsigned char keystore_hash[32] = {
		0xAA, 0x3B, 0x24, 0x94, 0xD7, 0xB8, 0x05, 0x42,
	...
	unsigned char gatekeeper_hash[32] = {
		0xAF, 0x49, 0x6D, 0x17, 0x5E, 0x66, 0xC0, 0x45,
		0xEE, 0xFC, 0xC0, 0xA9, 0x0B, 0x04, 0x2E, 0xB2,
		0x32, 0x18, 0xA4, 0x9F, 0x73, 0xA3, 0x67, 0x29,
		0x16, 0xAD, 0x47, 0x90, 0x3F, 0x50, 0xA1, 0xA9};

	unsigned char huawei_fingerprint_hash[32] = {
		0x2F, 0x63, 0xF0, 0x29, 0x92, 0x51, 0x86, 0xB2,
		0xDF, 0xB3, 0xA3, 0x14, 0x15, 0xC3, 0xAD, 0x30,
		0x7E, 0x52, 0x75, 0x5A, 0xBC, 0x43, 0x7B, 0xAE,
		0x42, 0x3E, 0x9C, 0x38, 0xAB, 0x45, 0x52, 0xCB
	};

	unsigned char widevine_hash[32] = {
		0xE1, 0xE5, 0x73, 0x5C, 0x0C, 0x00, 0xA0, 0x0E,
		0x09, 0xCA, 0xFF, 0x44, 0x7A, 0xFA, 0xBB, 0x87,
		0x15, 0x3A, 0x16, 0x1E, 0xAC, 0x46, 0x09, 0xDB,
		0x25, 0xC4, 0xB3, 0x09, 0xE9, 0x41, 0x2E, 0x86
	};

	if (!strncmp(pkname, "/vendor/bin/hw/android.hardware.gatekeeper@1.0-service", 54))
		memcpy(hash_buf, gatekeeper_hash, MAX_SHA_256_SZ);

	if (!strncmp(pkname, "/vendor/bin/hw/android.hardware.keymaster@3.0-service", 53))
		memcpy(hash_buf, keystore_hash, MAX_SHA_256_SZ);

	if (!strncmp(pkname, "/vendor/bin/hw/vendor.huawei.hardware.biometrics.fingerprint@2.1-service", 72))
		memcpy(hash_buf, huawei_fingerprint_hash, MAX_SHA_256_SZ);

	if (!strncmp(pkname, "/vendor/bin/hw/android.hardware.drm@1.1-service.widevine", 56))
...
find_service:
	...
	/* Spoof the correct hash to make TEE happy */
#if 0
