...
enum SVC_SECBOOT_CMD_ID {
	SECBOOT_CMD_ID_INVALID = 0x0,	/**< Secboot Task ????ID*/
	SECBOOT_CMD_ID_COPY_VRL,	/**< Secboot Task ????VRL*/
	SECBOOT_CMD_ID_COPY_DATA,	/**< Secboot Task ????????*/
	SECBOOT_CMD_ID_VERIFY_DATA,	/**< Secboot Task ????*/
	SECBOOT_CMD_ID_RESET_IMAGE,	/**< Secboot Task ????SoC????*/
	SECBOOT_CMD_ID_COPY_VRL_TYPE,  /**< Secboot Task ????VRL????????SoC Type*/
	SECBOOT_CMD_ID_COPY_DATA_TYPE,	/**< Secboot Task ????????,??????SoC Type*/
	SECBOOT_CMD_ID_VERIFY_DATA_TYPE, /**< Secboot Task ????????????SoC Type????????????????SoC*/
	SECBOOT_CMD_ID_VERIFY_DATA_TYPE_LOCAL, /**< Secboot Task????????????????SoC Type,??????????????SoC*/
	SECBOOT_CMD_ID_COPY_IMG_TYPE,          /*<Secboot Task Copy img from secure buffer to run addr>*/
	SECBOOT_CMD_ID_BSP_MODEM_CALL,           /**< Secboot Task ????????????*/
...
enum SVC_SECBOOT_IMG_TYPE {
    ...
    ISP,
/*
miami c30??????????????????????????????modem??????miami_c30 Modem????????????
????????????????????????????????????miami??????????OS????????????????????????????????????miami??????????????????????
??????????????????http://3ms.huawei.com/hi/group/8729/wiki_5190309.html
*/
