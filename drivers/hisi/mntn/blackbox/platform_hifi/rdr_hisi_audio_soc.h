...
struct icc_channel_fifo {
	unsigned int magic;     /* fifo??????????????fifo?????? */
	unsigned int size;      /* fifo???? */
	unsigned int write;     /* fifo?????? */
	unsigned int read;      /* fifo?????? */
...
struct icc_channel {
	...
	unsigned int ready_recv;       /* ????FIFO??????: ????????FIFO???????? */
	unsigned int ipc_send_irq_id;  /* ????????????????????????ipc???? */
	unsigned int ipc_recv_irq_id;  /* ????????????????????????ipc???? */
	unsigned int func_size;        /* ????????(??????)???? */
	unsigned int fifo_recv;        /* ????fifo?????? */
	unsigned int fifo_send;        /* ????fifo?????? */
};
...
struct icc_control {
	...
	unsigned char channels[ICC_CHN_ID_MAX*4];  /* icc_channel???????????????? */
};
...
enum DRV_RESET_CALLCBFUN_MOMENT {
	DRV_RESET_CALLCBFUN_RESET_BEFORE,   /*Modem/HIFI ??????*/
	DRV_RESET_CALLCBFUN_RESET_AFTER,    /*Modem/HIFI ??????*/
	DRV_RESET_CALLCBFUN_RESETING,       /*Modem/HIFI ??????*/
