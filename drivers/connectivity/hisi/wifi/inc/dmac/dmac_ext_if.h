...
struct  mac_tx_ctl
{
    ...
    frw_event_type_enum_uint8               bit_en_event_type           :6;          /* ????:FRW_EVENT_TYPE_WLAN_DTX??FRW_EVENT_TYPE_HOST_DRX??????:??????????????????????netbuf???????????? */
    ...
    oal_uint8                               en_is_eapol                 :1;           /* ??????????EAPOL?? 1102???????? */
