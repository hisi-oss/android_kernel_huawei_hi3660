                ...
                oal_uint8                     uc_lock)
{
    ...
    {
        OAM_ERROR_LOG2(0,0,"oal_mem_alloc_enhanced::us_len[%d] > pst_mem_pool->us_max_byte_len[%d]",us_len, pst_mem_pool->us_max_byte_len);
        return OAL_PTR_NULL;
    }
                ...
                oal_uint8       uc_lock)
{
    ...
    {
        OAL_MEM_SPIN_UNLOCK_IRQRESTORE(pst_mem_subpool->st_spinlock, ul_irq_flag)
        OAM_ERROR_LOG1(0,0,"oal_mem_free_enhanced::OAL_MEM_STATE_FREE == pst_mem->en_mem_state_flag[%d]", pst_mem->en_mem_state_flag);
    ...
    {
        OAL_MEM_SPIN_UNLOCK_IRQRESTORE(pst_mem_subpool->st_spinlock, ul_irq_flag)
        OAM_ERROR_LOG2(0,0,"oal_mem_free_enhanced::pst_mem_subpool->us_free_cnt[%d] >= pst_mem_subpool->us_total_cnt[%d]",pst_mem_subpool->us_free_cnt, pst_mem_subpool->us_total_cnt);
    ...
    //         void* p2 = p1;
    //         OAL_MEM_FREE(p1);
    //         ((TYPE*)p2)->p_member.val = somevalue; //????p2??????????????????????????????????
    //         ??????????????????????????????????????????????????????????????????????????!
    //         ????????????????????????????????????????????????????????????????????!!!
    //
    //         ??????????????????????????????????????????????????????????????????????????????????????????????????
                ...
                oal_uint8     uc_lock)
{
    ...
    if(OAL_UNLIKELY(ul_ret != OAL_SUCC))
    {
        ...
        OAM_ERROR_LOG3(0, OAM_SF_ANY, "oal_mem_free fail,ul_ret=%d,file_id[%d],line_num[%d]",ul_ret,ul_file_id,ul_line_num);
#ifdef _PRE_DEBUG_MODE
        OAM_ERROR_LOG2(0,OAM_SF_ANY,"oal_mem_free:last free file_id[%d],line_num[%d]",pst_mem->ul_alloc_file_id[0],pst_mem->ul_alloc_line_num[0]);
#endif
        //OAL_WARN_ON(1);
    }
    return ul_ret;
