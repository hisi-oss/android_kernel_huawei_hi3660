                   ...
                   struct hcc_transfer_param* param)
{
    ...
    {
        oal_print_hi11xx_log(HI11XX_LOG_ERR, "%s error: hdr->pad_hdr + HCC_HDR_LEN:%lu > len:%d",__FUNCTION__,hdr->pad_hdr + HCC_HDR_LEN ,HCC_HDR_TOTAL_LEN);
        return  -OAL_EFAIL;
    };
...
oal_void hcc_dev_flowctrl_off(struct hcc_handler *hcc)
{
    ...
    if (D2H_MSG_FLOWCTRL_ON == hcc->hcc_transer_info.tx_flow_ctrl.flowctrl_flag)
    {
        oal_timer_start(&hcc->hcc_transer_info.tx_flow_ctrl.flow_timer, hcc->hcc_transer_info.tx_flow_ctrl.timeout);
