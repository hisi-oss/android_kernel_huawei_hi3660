...
oal_uint32  hmac_config_start_vap(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    else if (WLAN_VAP_MODE_BSS_STA == pst_mac_vap->en_vap_mode)
    {
       ...
       /*:hishare??p2p device??????gc????p2p device????????fake up??????????????????vap_ctrl????????gc mac??????????*/
   ...
   if(OAL_TRUE == pst_hmac_vap->bit_11r_enable)
   {
    ...
    if (MAC_VAP_STATE_UP == pst_mac_vap->en_vap_state)
    {
            ...
            && (OAL_STRLEN(pst_mac_vap->pst_mib_info->st_wlan_mib_sta_config.auc_dot11DesiredSSID) == pst_connect_param->uc_ssid_len))
       {
            ...
            {
                /* reassociation */
                return hmac_roam_start(pst_hmac_vap, ROAM_SCAN_CHANNEL_ORG_0, OAL_FALSE, ROAM_TRIGGER_APP); /* ????BSSID??????BSSID???????????????? */
            }
            else
            {
                /* roaming */
...
oal_uint32 hmac_config_set_ft_ies(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    /* ????????????????,hmac_roam_exit??pul_roam_info????OAL_PTR_NULL */
