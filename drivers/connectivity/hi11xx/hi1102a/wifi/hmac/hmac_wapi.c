...
oal_uint8 hmac_wapi_is_keyidx_valid(hmac_wapi_stru *pst_wapi, oal_uint8 uc_keyidx_rx)
{
    ...
    pst_wapi->uc_keyupdate_flg = OAL_FALSE; /* ???????????????? */
...
{
    oal_uint32           ul_ret;
    oal_netbuf_stru     *pst_netbuf_tmp;    /* ??????????????netbuff */
    oal_netbuf_stru     *pst_netbuf_prev;   /* ??????????????netbuff */
    oal_netbuf_stru     *pst_buf_first;     /* ??????????????netbuff */
    //oal_uint16           us_len;
...
oal_uint32 hmac_wapi_decrypt(hmac_wapi_stru *pst_wapi, oal_netbuf_stru  *pst_netbuf)
{
    ...
    oal_uint8                    *puc_pn;
    oal_uint8                    *puc_pdu;
    oal_uint8                    *puc_mic_data;        /* ??????????????mic???????????????? wapi???????? ??51 */
    oal_uint8                    *puc_mic;
    oal_uint16                    us_mic_len;
...
//#endif /* WAPI_DEBUG_MODE */
    ...
    if (0 != oal_memcmp(puc_mic, auc_calc_mic, SMS4_MIC_LEN))              /* ????MIC */
    {
//#ifdef _PRE_WAPI_DEBUG
...
oal_uint32 hmac_wapi_encrypt(hmac_wapi_stru *pst_wapi, oal_netbuf_stru  *pst_netbuf)
{
    ...
    oal_uint8                        auc_calc_mic[SMS4_MIC_LEN];
    oal_uint8                        auc_pn_swap[SMS4_PN_LEN]; /* ????????????pn,????????mic?????? */
...
//#endif /* WAPI_DEBUG_MODE */
	...
	/* ????put????????netbuff??len??mac??+pdulen+sms4+wapi?????? */
