                                             ...
                                             oal_uint8  uc_index)
{
    ...
    if (ul_new_tcp_ack_no == *pul_old_tcp_ack_no) /* ????????tcp ack?? */
    {
#ifdef _PRE_WLAN_TCP_OPT_DEBUG
                                             ...
                                             const hcc_chan_type dir)
{
    ...
    if(uc_last_used_idx >= HMAC_TCP_STREAM)/*  ?????????????????? */
    {
        /* ????last_used_idx????????,???????????????? */
        return OAL_FALSE;
    }
...
#endif
            pst_tmp_tcp_pool[uc_tcp_stream_index].ui_last_ts = jiffies;   /* ?????????????????????????????? */
