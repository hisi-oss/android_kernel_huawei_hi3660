...
OAL_STATIC oal_uint32 wal_hipriv_set_protocol_debug_info(oal_net_device_stru *pst_net_dev, oal_int8 *pc_param)
{
    ...
    /* sh hipriv.sh "wlan0 protocol_debug band_force_switch 0|1|2(20|40+|40-) csa [mode 1:???????? 0:??????]0 [channel]1 [bandwidth]0 [cnt]2 [type]0=???? 1=???? 2=????" */
...
OAL_CONST wal_hipriv_cmd_entry_stru  g_ast_hipriv_cmd_debug[] =
{
    ...
    {"2040_coext_switch",       wal_hipriv_set_2040_coext_switch},       /* ????mac_vap_cap_flag: hipriv "vap0 2040_coext_switch 0|1" */
    ...
    {"wmmac_switch",            wal_hipriv_wmmac_switch},           /* ????????TS??????????: hipriv "vap0 wmmac_switch 1/0(????) 0|1(WMM_AC????????) AC xxx(limit_medium_time)" */
#endif
   ...
   // {"reset_hw",                wal_hipriv_reset_device},           /* ????????phy&mac: hipriv "Hisilicon0 reset_hw 0|1|2|3(all|phy|mac|debug) 0|1(reset phy reg) 0|1(reset mac reg) */
    ...
    {"napi_weight",             wal_hipriv_napi_weight},            /* hipriv "wlan0 napi_weight 0|1(??????) 16(weight)"*/
    ...
    {"phy_debug",             wal_hipriv_set_phy_debug_switch},        /* ????????????????rssi????????????sh hipriv.sh "Hisilicon0 rssi_switch 0|1(????|????) N(????N????????)" */
#if (_PRE_MULTI_CORE_MODE_OFFLOAD_DMAC == _PRE_MULTI_CORE_MODE)
#ifdef _PRE_DEBUG_MODE
    ...
    {"protocol_debug",                 wal_hipriv_set_protocol_debug_info},   /* ????????phy????????????????sh hipriv.sh "wlan0 protocol_debug [band_force_switch 0|1|2] [csa 0|1 [channel] [bandwidth] [cnt] 0|1|2]" */
