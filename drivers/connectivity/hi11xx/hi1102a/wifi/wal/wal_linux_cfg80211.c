                                                                ...
                                                                oal_vif_params_stru *pst_params)
{
    ...
    pst_net_dev = oal_net_alloc_netdev_mqs(OAL_SIZEOF(oal_netdev_priv_stru), auc_name, oal_ether_setup, WAL_NETDEV_SUBQUEUE_MAX_NUM, 1);    /* ??????????????????????????????????NAPI??????*/
#elif defined(_PRE_WLAN_FEATURE_OFFLOAD_FLOWCTL)
    pst_net_dev = oal_net_alloc_netdev_mqs(OAL_SIZEOF(oal_netdev_priv_stru), auc_name, oal_ether_setup, WLAN_NET_QUEUE_BUTT, 1);    /* ??????????????????????????????????NAPI?????? */
#else
    pst_net_dev = oal_net_alloc_netdev(OAL_SIZEOF(oal_netdev_priv_stru), auc_name, oal_ether_setup);    /* ??????????????????????????????????NAPI?????? */
#endif
    if (OAL_UNLIKELY(OAL_PTR_NULL == pst_net_dev))
    {
                                                    ...
                                                    oal_uint64               *pull_cookie)
{
    ...
    if (g_st_dfr_info.bit_device_reset_process_flag)
    {
        OAM_WARNING_LOG1(0, OAM_SF_ANY, "{wal_cfg80211_remain_on_channel:: dfr_process_status[%d]!}",
            g_st_dfr_info.bit_device_reset_process_flag);
        return -OAL_EFAIL;
    }
#endif //#ifdef _PRE_WLAN_FEATURE_DFR
    ...
    ***************************************************************************/
    ...
    if(OAL_SUCC != ul_err_code)
    {
        OAM_WARNING_LOG1(0, OAM_SF_CFG, "{wal_cfg80211_remain_on_channel::wal_check_and_release_msg_resp fail return err code:[%u]!}\r\n",
                         ul_err_code);
        return -OAL_EFAIL;
    }
                                            ...
                                            oal_uint64                  *pull_cookie)
{
    ...
    *pull_cookie = pst_p2p_info->ull_send_action_id++;   /* cookie????????????????????????????????????????callback */
    ...
    do
    {
        ...
        uc_retry++;
    }while((ul_ret != OAL_SUCC) && (uc_retry <= WAL_MGMT_TX_RETRY_CNT) && time_before(OAL_TIME_JIFFY, ul_start_time_stamp + OAL_MSECS_TO_JIFFIES(2 * WAL_MGMT_TX_TIMEOUT_MSEC)));
