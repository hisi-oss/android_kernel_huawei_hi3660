...
typedef enum
{
    ...
    *************************************************************************/
    ...
    WLAN_CFGID_PHY_DEBUG_SWITCH             = 169,       /* ??????????????rssi?????????????? */
    ...
    WLAN_CFGID_SET_POW_RF_CTL               = 223,      /* ??????????????RF?? */
#if (_PRE_MULTI_CORE_MODE_OFFLOAD_DMAC != _PRE_MULTI_CORE_MODE)
    ...
    WLAN_CFGID_SET_DEVICE_FREQ_ENABLE   = 573,   /* ????????????????*/
#endif
    ...
    WLAN_CFGID_SET_APF      = 581,         /* ????APF PROGRAM */
#endif
#if (_PRE_MULTI_CORE_MODE_OFFLOAD_DMAC == _PRE_MULTI_CORE_MODE)
    WLAN_CFGID_PM_DEBUG_SWITCH          = 582,         /* ?????????????? */
#endif
    ...
    WLAN_CFGID_SET_CUS_DYN_CALI_PARAM   = 599,      /* ????dyncali ???? */
#endif
    ...
    WLAN_CFGID_SET_CUS_BASE_POWER,                  /* ???????????? */
    WLAN_CFGID_SET_CUS_FCC_CE_POWER,                /* ?????????????????????? */
    ...
    WLAN_CFGID_SET_CUS_DOWNLOAD_RATE_LIMIT  = 663,           /* ?????????????? */
#endif
#ifndef CONFIG_HAS_EARLYSUSPEND
    WLAN_CFGID_SET_SUSPEND_MODE             = 664,           /* ?????????????? */
#endif

#ifdef _PRE_WLAN_FEATURE_TCP_ACK_BUFFER
    WLAN_CFGID_TCP_ACK_BUF                  = 665,
#endif
    WLAN_CFGID_SET_ALWAYS_TX_NUM            = 666,           /* ????1102A????????????*/

    WLAN_CFIGD_MCS_SET_CHECK_ENABLE         = 667,           /* ????????mcs ?????? */
...
#endif
    WLAN_CFGID_BINDCPU                      = 672,            /* ???????? */
    WLAN_CFGID_NAPIWEIGHT                   = 673,            /* ????NAPI weight?? */
    ...
    *************************************************************************/
    ...
    WLAN_CFGID_DYN_EXTLNA_BYPASS_SWITCH = 2011,  /* ????bypass????LNA???? */
#endif

    WLAN_CFGID_SET_TX_POW               = 2012,  /* ????HAL?????????? */
    WLAN_CFGID_VAP_MIB_UPDATE           = 2013,  /* d2h ????hal cap??VAP mib ??????????host?? */
