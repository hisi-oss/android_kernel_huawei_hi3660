...
typedef enum
{
    MAC_BTCOEX_CFG_ONEPKT_TYPE, //??????????????one pkt????????
    MAC_BTCOEX_CFG_RSP_CTS,     //??????????????????????CTS
    MAC_BTCOEX_CFG_BA_SIZE,     //????????????????????
    MAC_BTCOEX_CFG_BUTT
...
typedef struct
{
    ...
    oal_bool_enum_uint8           en_tsensor_debug_switch;     /* Tsensor?????????????? */
    ...
    oal_uint8                     uc_extlna_chg_bypass_switch; /* ????bypass????LNA????: 0/1/2:no_bypass/dyn_bypass/force_bypass */
#endif
...
typedef struct
{
    ...
    oal_bool_enum_uint8                 en_ht_capable;                      /* ????????ht */
    oal_bool_enum_uint8                 en_vht_capable;                     /* ????????vht */
    oal_bool_enum_uint8                 en_epigram_vht_capable;              /* ????????hidden vendor vht */
    oal_bool_enum_uint8                 en_epigram_novht_capable;            /* ????vendor????????????vht */
    ...
    oal_uint8                           uc_wapi;
    oal_uint8                           auc_resv[1];
    oal_bool_enum_uint8                 en_btcoex_blacklist_chip_oui;       /* ps????one pkt????????????????self-cts?? */
...
{
    oal_uint32                              ul_cmd_bit_map;
    oal_bool_enum_uint8                     en_band_force_switch;    /* ????????????*/
    oal_uint8                               auc_resv[3];
