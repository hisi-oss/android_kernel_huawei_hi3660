...
#define WLAN_TXBF_MATRIX_BUFFER_SIZE            (512)    /* ??buffer unit??????468byte = 16*2/2*234/8(80M),??????snr??????10byte,??????????512 */
...
#define HAL_MAX_CHAIN_NUM                 1  /* ???????????? */
#define HAL_TPC_UPC_RF_LUT_NUM            128 /* UPC??RF???????????? */
...
#define BTCOEX_PRIO_TIMEOUT_150MS           (150)    //????????????????????ms
