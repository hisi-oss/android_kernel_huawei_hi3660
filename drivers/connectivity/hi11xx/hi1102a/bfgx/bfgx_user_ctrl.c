...
STATIC ssize_t store_octty_test(struct device *dev, struct kobj_attribute *attr, const char *buf, size_t count)
{
    ...
    switch (cmd)
    {
        ...
        case 2:
            ...
            break;
        case 3:
            result = ps_change_uart_baud_rate(WKUP_DEV_BAUD_RATE, FLOW_CTRL_DISABLE);// BUAD RATE:115200, flow control: disable
            break;
        case 4:
            result = ps_change_uart_baud_rate(LOW_FREQ_BAUD_RATE, FLOW_CTRL_ENABLE);// BUAD_RATE:4000000, flow control:enable
        default:
            PS_PRINT_ERR("unknown cmd %d\n", cmd);
            break;
    }
