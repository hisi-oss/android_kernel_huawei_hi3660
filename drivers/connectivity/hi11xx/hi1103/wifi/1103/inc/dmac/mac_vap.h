...
/* 11N MCS?????????? */
#if 0
#define MAC_MAX_RATE_SINGLE_NSS_20M_11N 72  /* 1????????20MHz??????????*/
#define MAC_MAX_RATE_SINGLE_NSS_40M_11N 150 /* 1????????40MHz??????????*/
#define MAC_MAX_RATE_DOUBLE_NSS_20M_11N 144 /* 1????????80MHz??????????*/
#define MAC_MAX_RATE_DOUBLE_NSS_40M_11N 300 /* 2????????20MHz??????????*/
#endif
...
#define MAC_MAX_RATE_SINGLE_NSS_20M_11AC  107 /* 1????????20MHz??????Long GI????*/
#define MAC_MAX_RATE_SINGLE_NSS_40M_11AC  225 /* 1????????40MHz??????Long GI????*/
#define MAC_MAX_RATE_SINGLE_NSS_80M_11AC  487 /* 1????????80MHz??????Long GI????*/
#define MAC_MAX_RATE_SINGLE_NSS_160M_11AC 975 /* 1????????160MHz??????Long GI????*/
#define MAC_MAX_RATE_DOUBLE_NSS_20M_11AC  216 /* 2????????20MHz??????Long GI????*/
#define MAC_MAX_RATE_DOUBLE_NSS_40M_11AC  450 /* 2????????40MHz??????Long GI????*/
#define MAC_MAX_RATE_DOUBLE_NSS_80M_11AC  975 /* 2????????80MHz??????Long GI????*/
#define MAC_MAX_RATE_DOUBLE_NSS_160M_11AC 1950 /* 2????????160MHz??????Long GI????*/
#else
/* ????????????(9.4.2.158.3????)????????long gi???? */
...
#define MAC_MAX_RATE_SINGLE_NSS_160M_11AC 780 /* 1????????160MHz??????????*/
...
#define MAC_MAX_RATE_DOUBLE_NSS_160M_11AC 1560 /* 2????????160MHz??????????*/
#endif
#define HMAC_FBT_RSSI_ADJUST_VALUE         100
#define HMAC_RSSI_SIGNAL_INVALID           0xff        /*????RSSI??????*/
#define HMAC_FBT_RSSI_MAX_VALUE            100

#define MAC_VAP_USER_HASH_INVALID_VALUE      0xFFFFFFFF                         /* HSAH?????? */
#define MAC_VAP_USER_HASH_MAX_VALUE         (WLAN_ASSOC_USER_MAX_NUM * 2)       /* 2?????????? */
...
#define MAC_EXT_PA_GAIN_MAX_LVL              2   /* ??????????PA?????????? */
#define CUS_RF_PA_BIAS_REG_NUM              43   /* ??????RF????PA???????????????? */
...
#define CUS_DY_CALI_5G_VAL_DPN_MAX         (50)  /* ????????5g dpn????nvram?????? */
#define CUS_DY_CALI_5G_VAL_DPN_MIN         (-50) /* ????????5g dpn????nvram?????? */
...
#endif
#define HMAC_RSP_MSG_MAX_LEN            128   /* get wid???????????????? */
...
typedef enum
{
    ...
    MAC_TEST_FTM         =  13,/* FTM?????? */
    MAC_TEST_MGMT_ACTION = 14,/*????????Action??*/
    MAC_TEST_MGMT_BEACON_INCLUDE_IE = 15,/*Beacon ????????????????IE????????*/
...
typedef enum
{
    ...
    MAC_TAS_RSSI_CALI_DEBUG,        /* TAS????????  */
    MAC_TAS_ANT_SWITCH,             /* TAS????????  */
#endif
    MAC_SET_POW_BUTT
...
#define WAL_HIPRIV_RATE_INVALID       255 /*  ??????????????  */
...
typedef struct
{
    ...
    oal_uint8                            uc_sta_csa_last_cnt;          /*???? ap???????????????????? */
    oal_bool_enum_uint8                  en_is_fsm_attached   : 4;       /*??????????????????*/
    oal_bool_enum_uint8                  en_expect_cnt_status : 4;
    oal_uint8                            uc_associate_channel;         /*csa????????probe response ??????*/
    oal_uint8                            auc_resv[3];                  /*resv*/
    oal_fsm_stru                         st_oal_fsm;                   /*csa??????*/
...
typedef struct
{
    oal_bool_enum_uint8  en_include_opmode_notify_ie;     /* Beacon??????????????????????opmode_notify IE */
    oal_bool_enum_uint8  en_bf_rx_nss_change;             /* Beamforming??Rx Nss???? */
    oal_uint8            uc_include_ie_count;             /* Beacon??????????????????opmode_notify IE???? */
...
typedef struct
{
    oal_uint32  bit_uapsd                          : 1,
                ...
                bit_1024qam                        : 1,                 /* ????????1024qam????: 0-??????, 1-???? */
                bit_wmm_cap                        : 1,                 /* ??????STA??????AP????????wmm???????? */
                bit_is_interworking                : 1,                 /* ??????STA??????AP????????interworking???? */
                bit_ip_filter                      : 1,                 /* rx????ip???????????? */
                bit_opmode                         : 1,                 /* ??????probe rsp????extended capbilities ????????OPMODE */
                bit_nb                             : 1,                 /* ???????????????? */
                bit_2040_autoswitch                : 1,                 /* ??????????????????2040???????? */
                bit_2g_custom_siso                 : 1,                 /* 2g????????????????siso,????????0,?????????? */
                bit_5g_custom_siso                 : 1,                 /* 5g????????????????siso,????????0,?????????? */
                bit_bt20dbm                        : 1,                 /* 20dbm??????????????host??sta???????????? */
                bit_resv                           : 3;
}mac_cap_flag_stru;
...
typedef struct
{
    oal_bool_enum_uint8                 en_11ax_cap;            /*????????11ax????*/
...
#else
    oal_uint8              uc_assoc_sta_member_nums;    /* ??????????STA???? */
...
typedef struct
{
    ...
    oal_uint8            auc_reserve[2];
#else
    oal_uint8            uc_flag;                           /*flag??????flag??bit0??1????????bit1??1????????*/
    oal_uint8            uc_resp_nums;                      /*????????probe resp??????*/
#endif
...
typedef struct
{
    ...
    oal_uint8  auc_rev[2];
    mac_fbt_disable_user_info_stru ast_fbt_disable_connect_user_list[HMAC_FBT_MAX_USER_NUM];  /* ????32??????????????????????32????????????????????3?????? */
...
{
    oal_uint8 uc_channel_usage;                     //????????40%??????40
    oal_uint8 uc_offset_second_channel;    //????????
    oal_uint8 auc_hisi_cca_shreshold[2];         //??????????
    oal_uint32 ul_hisi_dig;                     //????????
    oal_uint16 us_fa;                      /* invalid frame - crc err .etc*/  //1????????????????????????????
    oal_uint16 us_cca;                     /* valid frame, can be recongnized*/ //1??????????????,????????????
    oal_uint32 ul_wait_agg_time;           //??????????????ms)
    oal_uint32 ul_channel_busy;
    /* buffer use status */
    oal_uint32 ul_skb_remain_buffer;       //skb????buff   WLAN_TID_MPDU_NUM_LIMIT- mac_device_stru->us_total_mpdu_num
    oal_uint32 ul_vo_remain_count;         //vo????????buff WLAN_TID_MPDU_NUM_LIMIT- mac_device_stru->us_total_mpdu_num
    oal_uint32 ul_vi_remain_count;         //vi????????buff
    oal_uint32 ul_be_remain_count;         //be????????buff
    oal_uint32 ul_bk_remain_count;         //bk????????buff
    /* edca setting  */
    oal_uint32 ul_vo_edca;                 //1151??????????????
...
{
    void *pst_mac_usr;
    oal_uint8 uc_agg_mode;                 //??????1?? 0??????
    oal_uint8 uc_max_agg_num;              //????????????
    oal_uint8 uc_rts_rate;                 /* MB/s*/ //????RTS??????
    oal_uint8 uc_rts_retry_cnt;            //????RTS??????????
    oal_int8 c_rssi;
    oal_int8 ac_reserved[3];
    oal_uint32 ul_tx_unicast_bytes;        /* becareful invert */  //??????????????????
    oal_uint32 ul_rx_unicast_bytes;        //??????????????????
    oal_uint32 ul_tx_mcast_bytes;          /* ??????????????????: ?????????????????? */
    oal_uint32 ul_rx_mcast_bytes;          /* ??????????????????: ?????????????????? */
    oal_uint32 aul_sta_tx_mcs_cnt[16];     /* how many tx mcs count  ????????????????????MCS??????,????0 */
    oal_uint32 aul_sta_rx_mcs_cnt[16];     /* how many rx mcs count ????????????????????MCS??????,????0  */
    oal_uint32 ul_sleep_times;             //????????????
...
{
    NO_POWERSAVE     = 0,
    MIN_FAST_PS      = 1,       /* idle ????????ini????????*/
    MAX_FAST_PS      = 2,       /* idle ????????ini????????*/
    AUTO_FAST_PS     = 3,       /* ????????????????idle??????ini??????????????????*/
    MAX_PSPOLL_PS    = 4,
    NUM_PS_MODE      = 5
} ps_user_mode_enum;
...
typedef struct
{
    mac_eid_enum_uint8              en_eid;                 /* ??????????element id */
    oal_ie_set_type_enum_uint8      en_set_type;            /* IE?????????? */
    oal_uint16                      us_ie_content_len;      /* IEdata??????????,????HDR???? */
    oal_uint8                       auc_ie_content[0];      /* IE DATA????????,???????? */
}mac_vap_ie_set_stru;
...
typedef struct
{
    ...
    oal_uint8                        auc_qos_enhance_mac[WLAN_MAC_ADDR_LEN];/* qos_enhance??????mac???? */
    oal_uint8                        uc_delete_num;                         /* ????????delete???????? */
    oal_uint8                        uc_add_num;                            /* ????????add???????? */
...
typedef struct
{
    ...
    oal_bool_enum_uint8               en_qos_enhance_enable; /* ????qos enhance???????? */
    oal_uint8                         uc_reserv[2];
...
typedef struct
{
    ...
    oal_uint8                           bit_one_rx_tcp_be       :   1;                  /* ??vap????????1??????TCP BE???? */
    oal_uint8                           bit_no_tcp_or_udp       :   1;                  /* ??vap??????TCP??UDP???? */
    oal_uint8                           bit_bw_fixed            :   1;                  /* AP??????VAP??????????????20M */
    oal_uint8                           bit_use_rts_threshold   :   1;                  /* ??vap??RTS??????????????????????RTS???? */
    ...
    oal_uint16                          us_mgmt_frame_filters;                    /* ???????????????? en_fbt_mgmt_frame_filter_enum */
    ...
    oal_uint32                          ul_fastip_idx;                                 /* ????????netdev?????? ?? fastip index */
#endif
...
#if 0
    mac_vap_dft_stru                    st_vap_dft;                                     /* vap???????????????????? */
#endif
    ...
    oal_bool_enum_uint8                 en_dhcp_req_disable_switch;                     /* dhcp req disable????????*/
    oal_uint8                           auc_resv8[3];  /* 4???????? */
#endif

#ifdef _PRE_WLAN_FEATURE_11KV_INTERFACE
    mac_vap_ie_set_stru          *pst_rrm_ie_info;                                      /* ????rrm ie :70 */
    mac_vap_ie_set_stru          *pst_excap_ie_info;                                    /* ????extend capbility ie: 127 */
    mac_vap_ie_set_stru          *pst_msta_ie_info;                                     /* ????Multi-STA 4????ie */
#endif
    ...
    oal_bool_enum_uint8                 en_40M_intol_user;                               /* ap??????????????40M intolerant??user*/
    oal_bool_enum_uint8                 en_vap_wmm;                                      /* ap wmm ???? */
#ifdef _PRE_WLAN_PRODUCT_1151V200
    oal_uint8                           uc_1st_asoc_done;                               /* ?????????? */
    oal_uint8                           uc_force_resp_mode_80M;                         /* sifs??????????80M???? */
#else
    ...
    mac_fbt_mgmt_stru                      st_fbt_mgmt;                               /* ????fbt?????????????????????? */
#endif
    ...
    oal_uint16                          us_assoc_vap_id;                                /* VAP??STA??????????user(ap)??????????????VAP??AP?????????????????????? */
...
//#ifdef _PRE_WLAN_FEATURE_ALWAYS_TX
    ...
    oal_uint8                           bit_ack_policy:1;   /* ack_policy????:0:normal 1:no ack , ??????????*/

    /* ???????????? */
    oal_uint8                           bit_al_rx_flag  :1; /* ???????? */
    oal_uint8                           bit_reserved:3;
//#endif
...
//#endif    /* _PRE_WLAN_FEATURE_P2P */
    ...
    oal_uint8                           auc_resv9[1];            /* 4???????? */
#endif
#ifdef _PRE_WLAN_FEATURE_DFS
    oal_bool_enum_uint8                 en_radar_detector_is_supp;/*????hal device????????????????????????*/
    oal_uint8                           auc_resv10[3];            /* 4???????? */
#endif
#ifdef _PRE_WLAN_FEATURE_QOS_ENHANCE
...
typedef struct
{
    ...
    oal_uint8                 uc_dst_hal_dev_id;      //vap??????hal device id
#else
...
#ifdef _PRE_WLAN_FEATURE_HILINK
    oal_uint8               uc_rej_user;                        /* ????sta????, TRUE:????????STA, FALSE:????????STA */
    oal_uint8               uc_kick_user;                       /* ????:????/???????? TRUE:????sta, */
    oal_uint8               uc_mlme_phase_mask;                 /* ???????????? */
    oal_uint8               uc_rsv;
#endif
...
typedef struct
{
    mac_car_type_enum_uint8         en_car_flag;           //7??????????
    oal_uint8                       uc_car_up_down_type;   //??????0????????1
...
typedef struct
{
    mac_rssi_limit_type_enum_uint8  en_rssi_limit_type;           //4??????????
...
typedef struct
{
    ...
    mac_delba_trigger_enum_uint8    en_trigger;                         /* ???????? */
...
typedef struct
{
    ...
    oal_uint16                             us_timeout_period;/*??????????????*/
    oal_uint8                              uc_factor;/*??????????medium_time/2^factor */
    oal_uint8                              auc_rsv[3];
...
typedef struct
{
    oal_uint8                        bit_switch : 1,
                                     bit_dev_id : 1,
                                     bit_flag   : 2, /* 0/1/2/3 ??????????????0:??0??1:??1??2:?????? 3:????uc_content???????? */
                                     bit_rsv    : 4;
...
/* ??mac_cfg_lpm_wow_en_stru????????_PRE_WLAN_CHIP_TEST??????????????_PRE_WLAN_CHIP_TEST?????????????? */
...
typedef struct
{
    oal_uint16                  us_lna_on2off_time_ns;           /* LNA????LNA????????(ns) */
    oal_uint16                  us_lna_off2on_time_ns;           /* LNA????LNA????????(ns) */

    oal_int8                    c_lna_bypass_gain_db;         /* ????LNA bypass????????(dB) */
    oal_int8                    c_lna_gain_db;                /* ????LNA????(dB) */
    oal_int8                    c_pa_gain_b0_db;              /* ????PA b0 ????(dB) */
    oal_uint8                   uc_ext_switch_isexist;        /* ????????????switch */

    oal_uint8                   uc_ext_pa_isexist;            /* ????????????pa */
    oal_uint8                   uc_ext_lna_isexist;           /* ????????????lna */
#if (_PRE_PRODUCT_ID == _PRE_PRODUCT_ID_HI1103_HOST)
    oal_int8                    c_pa_gain_b1_db;              /* ????PA b1????(dB) */
    oal_uint8                   uc_pa_gain_lvl_num;            /* ????PA ?????????? */
#else
    oal_uint8                   auc_resv[2];
#endif
...
typedef struct
{
    ...
    oal_uint32                  aul_2g_pa_bias_rf_reg[CUS_RF_PA_BIAS_REG_NUM];   /* ??????RF????PA??????????reg236~244, reg253~284 */
#endif
}mac_cfg_customize_rf;

typedef struct
{
    oal_uint16                  us_tx_ratio;                        /* tx?????? */
    oal_uint16                  us_tx_pwr_comp_val;                 /* ?????????????? */
}mac_tx_ratio_vs_pwr_stru;

/* ??????TX??????&?????????????????????? */
typedef struct
{
    mac_tx_ratio_vs_pwr_stru    ast_txratio2pwr[3];                 /* 3?????????????????????????? */
    oal_uint32                  ul_more_pwr;                        /* ?????????????????????????? */
...
{
    /* dts */
    oal_int16      aus_cali_txpwr_pa_dc_ref_2g_val_chan[MAC_2G_CHANNEL_NUM];  /* txpwr??????ref?? */
    oal_int16      aus_cali_txpwr_pa_dc_ref_5g_val_band[MAC_NUM_5G_BAND];
#if (_PRE_TARGET_PRODUCT_TYPE_E5 == _PRE_CONFIG_TARGET_PRODUCT)
    oal_int16      aus_cali_txpwr_pa_dc_ref_2g_b40_val[MAC_2G_CHANNEL_NUM];   /* E5-5885 B40????ref?? */
...
typedef  struct
{
    ...
    oal_int8                         c_snr_ant0;                                /* ????0??????SNR?? */
    oal_int8                         c_snr_ant1;                                /* ????1??????SNR?? */
...
{
    oal_uint8  auc_mac_addr[OAL_MAC_ADDR_LEN];
    oal_en_sensing_bssid_operate_uint8  en_operation;   /** 0?????? 1???? */
    oal_uint8  reserved;
...
typedef struct
{
    ...
    oal_uint8    uc_rx_buffer_size;     /* addba rsp?????????????? */
    oal_uint16   us_resv;
...
typedef struct
{
    oal_uint8    uc_ini_en;             /* ??????????????????bypass extLNA???? */
    oal_uint8    uc_cur_status;         /* ???????????????????????? */
...
OAL_STATIC OAL_INLINE oal_bool_enum_uint8  mac_mib_get_FortyMHzOperationImplemented(mac_vap_stru *pst_mac_vap)
{
    return (WLAN_BAND_2G == pst_mac_vap->st_channel.en_band) ?
            ...
            pst_mac_vap->pst_mib_info->st_wlan_mib_phy_ht.en_dot115GFortyMHzOperationImplemented;/* [false alarm]:????????oal_bool_enum_uint8 ??????????????*/
}
...
OAL_STATIC OAL_INLINE oal_bool_enum_uint8 mac_mib_get_ShortGIOptionInFortyImplemented(mac_vap_stru *pst_mac_vap)
{
    return (WLAN_BAND_2G == pst_mac_vap->st_channel.en_band) ?
            ...
            pst_mac_vap->pst_mib_info->st_wlan_mib_phy_ht.en_dot115GShortGIOptionInFortyImplemented;/* [false alarm]:????????oal_bool_enum_uint8 ??????????????*/
}
