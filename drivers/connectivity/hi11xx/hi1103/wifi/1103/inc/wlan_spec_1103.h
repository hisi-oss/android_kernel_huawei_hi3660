...
#define WLAN_DEFAULT_DFS_CHANNEL_SCAN_TIME      240         /*GC??????????????????????????????*/
#else
...
#else
/* ????ms */
#define WLAN_AP_USER_AGING_TIME                 (60 * 1000)    /* AP ???????????? 60S */
#define WLAN_P2PGO_USER_AGING_TIME              (60 * 1000)     /* GO ???????????? 60S */
#endif

/* AP keepalive????,????ms */
#define WLAN_AP_KEEPALIVE_TRIGGER_TIME          (15 * 1000)       /* keepalive?????????????? */
#define WLAN_AP_KEEPALIVE_INTERVAL              (25 * 1000)   /* ap????keepalive null?????? */
...
#define WLAN_AMPDU_TX_MAX_NUM           64  /* AMPDU????????????????MPDU???? */
...
/* 1103 spec amsdu??????????????WLAN_LARGE_NETBUF_SIZE????????????buffer???? */
...
/*****************************************************************************
  2.5.9 DEVICE ????????????????
*****************************************************************************/
#define WLAN_RX_INTERRUPT_MAX_NUM_PER_DEVICE  16          /* ????device????????????16??rx???? */
...
#define WLAN_TX_QUEUE_UAPSD_DEPTH    5 /* ????????????BK????????????????5?????? */
...
/* HAL DEV0????support tx stbc, su/mu txbfer */
#if (WLAN_SINGLE_NSS == WLAN_MAX_NSS_NUM)
...
/* HAL DEV1?????????? */
#define WLAN_HAL1_PHY_CHAIN_SEL      WLAN_PHY_CHAIN_ZERO  /* ??phy????????????????1?????????? */
...
#define WLAN_VAP_MAX_ID_PER_DEVICE_LIMIT        5    /* hal????0 1 2 3 4 */
...
#define HT_BFEE_NTX_SUPP_ANTA_NUM              (4)     /* 11n??????bfer????sounding???????????????? */
/*****************************************************************************
  2.10.2 vht cap info
*****************************************************************************/
#define VHT_TX_STBC_DEFAULT_VALUE               0
#define VHT_BFEE_NTX_SUPP_STS_CAP              (4)     /* ??????????????????NDP Nsts???? */

/*****************************************************************************
  2.10.3 RSSI
*****************************************************************************/
#define WLAN_NEAR_DISTANCE_RSSI        (-20)             /*??????????????????-35dBm*/
#define WLAN_NEAR_DISTANCE_ADJUST_RSSI (15)             /*??????????????15dB*/
#define WLAN_CLOSE_DISTANCE_RSSI       (-15)             /*??????????????????-25dBm*/
...
#define WLAN_RSSI_ADJUST_TH            (-88)             /* ????????RSSI??????????, ??????????rssi-=2 */
#define WLAN_PHY_EXTLNA_CHGPTDBM_TH    (-25)             /* ??????????????????????????-25dBm */
#define WLAN_LOW_RATE_OPEN_NOISEEST_TH (200)             /* ??100ms?????????????????????????????????? */
#define WLAN_LOW_RATE_OPEN_MLD_TH      (100)             /* ??100ms????????????????MLD?????????? */
#define WLAN_LOW_RATE_OPEN_MLD_COUNT    (10)             /* ??100ms????????????????MLD?????????????? */
...
/* bfer 2*2??????????bfee report???? */
/* buffer size = 2*(4+6)/2*468/8(160M) = 590bytes,??????snr??????10byte,??????????????640bytes */
#define WLAN_TXBF_BFER_MATRIX_BUFFER_SIZE       (640)
/* bfee 4*2??????????mu bfee???????????? */
/* buffer size = 10*(7+9)/2*468/8(160M) = 4680bytes,??????snr??????10byte,??????????????5000bytes */
#define WLAN_TXBF_BFEE_MATRIX_BUFFER_SIZE       (5000)
/*****************************************************************************

  2.11 ??????????
*****************************************************************************/
#define WLAN_RX_DSCR_SIZE        WLAN_MEM_SHARED_RX_DSCR_SIZE      //??????????????????
...
#define BTCOEX_RX_STATISTICS_TIME           (2000)    //??????????????3s????????????????????1s??
