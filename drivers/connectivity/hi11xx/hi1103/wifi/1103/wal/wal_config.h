...
typedef struct
{
    wal_msg_hdr_stru            st_msg_hdr;         /* ?????????? */
    oal_uint8                   auc_msg_data[4];    /* ????????payload */
}wal_msg_stru;
...
typedef struct
{
    wlan_cfgid_enum_uint16  en_cfgid;                /* wid???? */
    oal_bool_enum_uint8     en_reset;              /* ???????? */
