...
typedef struct oal_vap_status_diag_info
{
    ...
    oal_uint8 uc_bandwidth;                //20M:20??40M:40??20/40M:60, ??????????????????????????????
    /* dynamic config parameters */
    oal_uint8 uc_dig;                       /*  reltek??????????????????0 */
    oal_uint8 uc_cca_shreshold;             /*  reltek??????????????????0 */
    oal_uint8 uc_channel_usage;                 //????????40%??????40  zhoukedou band steering
    oal_uint32 ul_hisi_dig;                     //???????? ??mcs???? 3.11
    oal_uint8 auc_hisi_cca_shreshold[2];        //??????????   3.10.1

    oal_uint8 uc_sta_cnt;                  //????????
    oal_uint8 uc_scan_mode;                //????????1??????????0  //mac_device_stru->en_curr_scan_state

    oal_uint16 us_fa;                      /* invalid frame - crc err .etc*/  //1????????????????????????????  hal_get_rx_err_count
    oal_uint16 us_cca;                     /* valid frame, can be recongnized*/ //1??????????????,????????????  //pst_hal_device->st_hal_device_base.ul_rx_normal_mdpu_succ_num++;
    oal_uint32 ul_channel_busy;            /** ??????????:(???????????? - ???????????? - ???????????? - ????????????????)*1000 / ???????????? */

    /* agg setting */
    oal_uint32 ul_wait_agg_time;           //??????????????ms)
    /* buffer use status */
    oal_uint32 ul_skb_remain_buffer;       //skb????buff
    oal_uint32 ul_vo_remain_count;         //vo????????buff
...
typedef struct oal_sta_status_diag_info{
    oal_uint8 auc_mac[ETH_ALEN];           /* sta??mac????: */
    oal_uint8 uc_db_mode;                  /* Mandatory: dualband mode: 2.4G only, 5G only, 2.4G and 5G dualband */
    oal_uint8 auc_chip_vendor[3];          /* ????????????: */
    oal_uint8 uc_wl_mode;                  /* ????????????: b,g,n,bg,bgn, hmac_user_stru.st_user_base_info.en_protocol_mode */
    oal_uint8 uc_agg_mode;                 /* ????????: ??bit????1??tid????????:1????????:0, dmac_user_stru.st_tid.pst_ba_tx_hdl->en_ba_conn_status */

    oal_uint8 uc_rssi;                     /* ????????????????: */
    oal_uint8 uc_snr;                      /* ??????: ????????????????????0 */
    oal_uint8 uc_tx_bandwidth;             /* ????????: hmac_user_stru.st_user_base_info.en_cur_bandwidth */
    oal_uint8 uc_rx_bandwidth;             /* ????????: hmac_user_stru.st_user_base_info.en_cur_bandwidth */
    oal_uint16 us_tx_rate;                 /* ????????????(MByte/s): hmac_user_stru.ul_tx_rate */
    oal_uint16 us_rx_rate;                 /* ????????????(MByte/s): hmac_user_stru.ul_rx_rate */
    oal_uint16 us_tx_minrate;              /* Mandatory: ????????????????????????????(??????????????sta??????????)MByte/s */
    oal_uint16 us_tx_maxrate;              /* Mandatory: ????????????????????????????(??????????????sta??????????)MByte/s */
    oal_uint32 ul_sleep_times;             /* ????????????: dmac_user_stru.ul_sta_sleep_times */
    oal_uint32 ul_tx_retry_cnt;            /* ????????????: oam_user_stat_info_stru.ul_tx_ppdu_retries, ?????? */
    oal_uint8 uc_per;                      /* ??????(????????)??40%??40 */
    oal_uint8 uc_max_agg_num;              /* ????????????: ????TID ??????????TID0, dmac_tid_stru.dmac_ba_tx_stru.uc_ampdu_max_num  */
    oal_uint8 uc_rts_rate;                 /* ????RTS??????(MB/s): hal_set_rts_rate_params */
    oal_uint8 uc_rts_retry_cnt;            /* ????RTS??????????:  dmac_tx_update_alg_param */

    oal_uint32 ul_tx_pkts;                 /* ??????????: oam_user_stat_info_stru */
    oal_uint32 ul_tx_fail;                 /* ??????????????: oam_user_stat_info_stru */
    oal_uint32 ul_rx_pkts;                 /* ??????????: oam_user_stat_info_stru.ul_rx_mpdu_num */
    oal_uint32 ul_tx_unicast_bytes;        /* ??????????????????: dmac_thrpt_stat_info_stru, becareful invert */
    oal_uint32 ul_rx_unicast_bytes;        /* ??????????????????: dmac_thrpt_stat_info_stru, */
    ...
    oal_uint32 ul_tx_spending;             /* Mandatory: ????????: ???????????? */
    oal_uint32 ul_tx_th;                   /* Mandatory: STA??1????tx?????? */
    oal_uint32 ul_tx_spending_smoothing_rate;  /* ?????????????? */
    oal_uint32 ul_current_tx_rate;         /* RTL????????????tx??????????????????????????????????????????????????*/
    oal_uint32 ul_tx_prev_rate;            /* Mandatory: STA????1????tx???????????????????rate????????????????????*/
} oal_sta_status_diag_info_stru;

typedef struct ieee80211req_wifi_rssi {
    oal_uint32   ul_timestamp;          /* ?????? */
    oal_int32    l_rssi;                /* rssi??  */
    oal_uint8    auc_local[ETH_ALEN];       /* AP??MAC???? */
    oal_uint8    auc_bssid[ETH_ALEN];       /* ??????RSSI AP??MAC???? */
...
typedef enum
{
    ...
    OAL_WME_AC_BUTT = 4,
    OAL_WME_AC_MGMT = OAL_WME_AC_BUTT,   /* ????AC??????????,????????????????????*/
...
typedef struct
{
    ...
    oal_uint8                           auc_bssid[OAL_MAC_ADDR_LEN];       /* ????bssid */
    oal_uint8                           uc_num_supp_rates;                  /* ???????????????? */
    oal_uint8                           auc_supp_rates[12];/* ???????????? */
    oal_uint8                           uc_channel;
    oal_uint8                           uc_bw;
    oal_uint8                           uc_max_nss;                         /* ??BSS?????????????????? */
    oal_uint8                           auc_resv[1];
    oal_uint8                           auc_ssid[OAL_IEEE80211_MAX_SSID_LEN];
    size_t                              ssid_len;
    struct cfg80211_crypto_settings     crypto;
    oal_uint32                          ul_max_rate_kbps;                   /* ??BSS??????????????(????:kbps) */
}oal_ap_info_stru;
...
typedef struct
{
    oal_uint32              ul_tx_succ_num[OAL_WME_AC_BUTT];                /*????????????*/
    oal_uint32              ul_rx_mpdu_num[OAL_WME_AC_BUTT];                    /*??????MPDU????*/
    oal_uint32              ul_tx_fail_num[OAL_WME_AC_BUTT];                /*??????????????????????????*/
    oal_uint32              ul_rx_fail_num[OAL_WME_AC_BUTT];
    oal_uint32              ul_tx_succ_bytes[OAL_WME_AC_BUTT];                  /*tx succ????*/
    oal_uint32              ul_rx_succ_bytes[OAL_WME_AC_BUTT];                  /*rx succ????*/
    oal_uint32              ul_tx_fail_bytes[OAL_WME_AC_BUTT];                  /*tx fail????*/
    oal_uint32              ul_rx_fail_bytes[OAL_WME_AC_BUTT];                  /*rx fail????*/
    oal_uint32              ul_forward_num[OAL_WME_AC_BUTT];
    oal_uint32              ul_forward_bytes[OAL_WME_AC_BUTT];                  /*forward????*/
    oal_uint32              ul_tx_expire[OAL_WME_AC_BUTT];
}oal_wme_stat_stru;

typedef struct
{
    oal_uint32                      ul_max_tx_delay[OAL_WME_AC_BUTT];                    /*????????????*/
    oal_uint32                      ul_min_tx_delay[OAL_WME_AC_BUTT];                    /*????????????*/
    oal_uint32                      ul_ave_tx_delay[OAL_WME_AC_BUTT];                    /*????????????*/
